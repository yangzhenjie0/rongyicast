...
import com.yxh.ryt.spannable.CircleMovementMethod;
import com.yxh.ryt.spannable.SpannableClickable;
...
public class CommentListView extends LinearLayout {
    ...
    private List<ArtworkComment> mDatas;
    ...
    public void setDatas(List<ArtworkComment> datas){
        if(datas == null ){
            datas = new ArrayList<ArtworkComment>();
        }
        mDatas = datas;
        notifyDataSetChanged();
    }

    public List<ArtworkComment> getDatas(){
        return mDatas;
    }
    ...
    private View getView(final int position){
        ...
        final ArtworkComment bean = mDatas.get(position);
        ...
        builder.append(setClickableSpan(bean.getCreator().getType(),name, bean.getCreator().getId()));

        if (!TextUtils.isEmpty(toReplyName)) {

            builder.append(" 回复 ");
            builder.append(setClickableSpan(bean.getFatherComment().getCreator().getType(),toReplyName, bean.getFatherComment().getCreator().getId()));
        }
        builder.append(": ");
        //转换表情字符
    ...
    @NonNull
    private SpannableString setClickableSpan(final String type, final String textStr, final String id) {
        SpannableString subjectSpanText = new SpannableString(textStr);
        subjectSpanText.setSpan(new SpannableClickable(itemColor){
                                    @Override
                                    public void onClick(View widget) {
                                        //Toast.makeText(AppApplication.getSingleContext(), textStr + " &id = " + id, Toast.LENGTH_SHORT).show();
                                        if ("1".equals(type)) {
